...
title: GoLang 逃逸分析
date: 2023/5/19
description: 深入GoLang逃逸分析，揭开底层运行机制，探索高效内存管理的奥秘。
tag: Backend Development, GoLang
author: Mika
---

import {} from './code-standard.mdx'

## 堆与栈
Go 会在 2 个地方为变量分配内存，
一个是全局的**堆（Heap）**，
另一个是每个 协程（Goroutine） 的栈（Stack）。
如果分配在栈中，函数执行结束时自动回收。
...
可以发现 `getName` 函数里面变量 `name` 逃逸了，这没有任何问题。
...
## 结论
指针传递可以减少值的拷贝，但是会导致内存分配逃逸到堆中，增加 GC 的负担。
值传递会拷贝整个对象，不会对 GC 产生负担。
